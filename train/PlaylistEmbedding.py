...
class PlaylistEmbedding:
    ...
    #file_path 는  모든 파일있는 제일 상위 path
    def __init__(self, FILE_PATH):
        self.FILE_PATH = '/Users/hansolyu/Downloads/멜론추천챗봇/감성분석음악추천챗봇' 
        ...
        train= pd.read_csv(FILE_PATH+'/melon_train_final.csv')
    ...
    def get_dic(self, train, val):
        ...
        for q in tqdm(data):
            ...
            intent_dic[str(q['id'])] = q['intent']
        self.song_dic = song_dic
        self.tag_dic = tag_dic
        self.intent_dic = intent_dic

        # 여기서 토탈로 train의 곡과 태그만 보내기 때문에 모델에는 train만 학습됨
        total = list(map(lambda x: list(map(str, x['songs'])) + list(x['tags']) + list(x['intent']), data))
    ...
    def get_result(self, p2v_model, song_dic, tag_dic, intent_dic, most_results, val, train, FT_title_model, song_meta):
        ...
        for n, q in tqdm(enumerate(val), total=len(val)):
            ...
            try:
                ...
                def ply_em(q):
                    ...
                    for intent in count3:
                        real_get_intent.append(tag[0])

                    # get_song = list(pd.value_counts(get_song)[:500].index)
                    # get_tag = list(pd.value_counts(get_tag)[:20].index)
                    # get_intent = list(pd.value_counts(get_intent)[:3].index)
    ...
    def run(self):
        ...
        # 곡과 태그 채우는 함수
        self.get_result(self.p2v_model, self.song_dic, self.tag_dic, self.intent_dic, self.most_results, self.val, self.train,
